...
CREATE POLICY "Company access" ON clients FOR ALL TO authenticated
...
CREATE POLICY "Company access" ON budgets FOR ALL TO authenticated
  USING (company_id = (SELECT id FROM companies WHERE id = (auth.jwt()->>'company_id')::uuid LIMIT 1));
...
CREATE TABLE IF NOT EXISTS cash_flow (
  ...
  collaborator_id UUID REFERENCES providers(id),
  paid BOOLEAN DEFAULT FALSE,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
ALTER TABLE cash_flow ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Company access" ON cash_flow FOR ALL TO authenticated
  USING (company_id = (SELECT id FROM companies WHERE id = (auth.jwt()->>'company_id')::uuid LIMIT 1));
