...
export default function Tasks({ showToast, sideFilter, user, plan, onStatsChange }) {
  ...
  function filtered() {
    ...
    const statusOrd = { em_andamento: 0, prestador_externo: 0, pendente: 1, cadastrada: 2, cancelada: 3, concluida: 4 }
  ...
  const tarefasHoje = tasks.filter(t => {
    if (['cancelada', 'cadastrada'].includes(t.status)) return false
