          ...
          return (
            ...
            >
              ...
              {dayTasks.slice(0, 3).map(t => (
                <div key={t.id} className={`cal-task-chip ${t.status === 'concluida' ? 'concluida' : t.urgency}`}>
                  #{t.id} {t.title}
                </div>
              ))}
              {dayTasks.length > 3 && <div style={{ fontSize: '.62rem', color: 'var(--muted)', marginTop: '.15rem' }}>+{dayTasks.length - 3} mais</div>}
            </div>
          )
        })}
      </div>
      ...
      {selected && (
        <div className="cfg-card" style={{ marginTop: '.85rem' }}>
          ...
          {selectedTasks.length === 0 ? (
            ...
            <div key={t.id} style={{ display: 'flex', gap: '.75rem', padding: '.6rem 0', borderBottom: '1px solid var(--border)', alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '.75rem', color: 'var(--muted)', marginTop: '.15rem' }}>
                  👤 {t.assignee} &nbsp;|&nbsp;
                  <span className={`ubadge ${t.urgency}`} style={{ padding: '.1rem .35rem' }}>{t.urgency}</span> &nbsp;|&nbsp;
                  <span className={`stbadge ${t.status}`} style={{ padding: '.1rem .35rem' }}>{t.status}</span>
                </div>
              </div>
            </div>
          ))}
